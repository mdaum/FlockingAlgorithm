...
to go ;;all turtles move one step...tick clock
  ...
  ask turtles [if wait? = false[ fd bot_speed / 60 + random (0.05 * (bot_speed / 60))] set wait? false] ;;just move forward...no reaction event
...
to separate
  ...
  if f_val < ir_z2 and f_val != 0[set separated? true set wait? true ]
...
to compute-r_val ;;in a pickle....i would imagine I should only react to people on my right side....pointing their heading towards me....
  let myTurtle turtle who let numMatter 0 ;;compute average distance of ppl triggering sensor
  rt 90
  let r_mates turtles in-cone sensor_range 90
  lt 90
  ask r_mates[
         if numMatter != 0 and numMatter > distance myTurtle [set numMatter distance myTurtle]
    ]
  if numMatter > 0 [set r_val numMatter + random (0.05 * numMatter)]
...
to compute-l_val
  let myTurtle turtle who let numMatter 0 ;;compute average distance of ppl triggering sensor
  lt 90
  let l_mates turtles in-cone sensor_range 90
  rt 90
  ask l_mates[
         if numMatter != 0 and numMatter > distance myTurtle [set numMatter distance myTurtle]
    ]
...
to compute-f_val
  let myTurtle turtle who let numMatter 0 ;;compute average distance of ppl triggering sensor
  let f_mates turtles in-cone sensor_range 90
  ask f_mates[
         if numMatter != 0 and numMatter > distance myTurtle [set numMatter distance myTurtle]
    ]
  if numMatter > 0 [set r_val numMatter + random (0.05 * numMatter)]
...
to compute-b_val
  let myTurtle turtle who let numMatter 0 ;;compute average distance of ppl triggering sensor
  lt 180
  let b_mates turtles in-cone sensor_range 90
  rt 180
  ask b_mates[
         if numMatter != 0 and numMatter > distance myTurtle [set numMatter distance myTurtle]
    ]
